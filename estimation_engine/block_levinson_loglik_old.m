function [x] = block_levinson_loglik_old(y, L, varargin) 
...
%           L -- dxdN or dNxd block vector of autocovariances or inverse
%                FFT population moments format
%                i.e. 
%                L=[L(1) L(2) ... L(N)]
%                or
%                L=[L(1)
...
%                   L(N)]
%                or
%                L=[L(1) L(2) ... L(N+1) L(N)' L(N-1)' ... L(2)']
%                where the difference between the first two is identified
%                by the dimensions of the block vector passed to the
%                function and the final version by passing a third argument
%                to the function.
...
lastwarn(''); %catch numerical probs
sz = size(L);
...
lndet=log(det(s_inv));          % first component of ln(det(G))
s=s_inv\eye(d,d);               % initial value for s 
if isempty(lastwarn)==0         % Catch a failure to invert s_inv and/or q_inv
x=-10^50;
return
end
...
siY=si*y(1:(N-1)*d);
x=x+y((N-1)*d+1:N*d)'*sy+siY'*sy+sy'*siY+siY'*s*siY;
x=-0.5*(N*d)*ln2pi-0.5*x-0.5*lndet;
