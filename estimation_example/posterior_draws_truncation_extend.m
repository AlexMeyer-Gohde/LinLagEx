%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%posterior_draws_truncation_extended.m
%
%This program:
%               is a script to get and assess draws from the posterior
%               distribution using five new independent chains for the
%               truncated model based on the first set of non-converged
%               chains
%
%      NOTE: The file posterior_mode_full.mat, containing the
%      results from a numerical maximization of the log posterior
%      likelihood function, as well as mixed_posteriors_truncation_3.mat,
%      containg a first set of mixed draws that didn't converge, must 
%      already exist. If this is not the case, run the file 
%      posterior_mode_truncation_3 and then posterior_draws_truncation_3 
%       first.
%
%       Simply change to the directory where this file is located, type
%       posterior_draws_truncation_extended, wait a couple of hours, and 
%       the results will be displayed and then saved in the file
...
mode_estimate_data='posterior_mode_truncation_3.mat';
save_final_name='mixed_posteriors_truncation_3_2';
...
'$\sigma^{eps}_{pc}$'
'$\sigma^{eps}_{mp}$'};
bayesian_plots_and_correlations_model_name='input_example_me_bayesian_correlations_truncation_3';
...
load('mixed_posteriors_truncation_3')
VARCOV=cov(mixed_results);
[ss sss]=max(mixed_densities);
posterior_mode=mixed_results(sss,:);

draw_type=0;
save_name='posterior_truncation_3_2_draw_1';
...
eval(sprintf('eval([''save_name=''''posterior_truncation_3_2_draw_%s'''';''])', num2str(j)))
[results,draws,results_densities,lagged_expectations]=metropolis_hastings(mode_estimate_data, draw_type, number_of_draws, burn_in, overdispersion,save_name);
