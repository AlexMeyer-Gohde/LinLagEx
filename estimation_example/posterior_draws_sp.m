...
%               sticky-price model
%
%      NOTE: The file posterior_mode_sp.mat, containing the
...
%      the file posterior_mode_sp first.
%
%       Simply change to the directory where this file is located, type
%       posterior_draws_sp, wait a couple of hours, and the results will be 
%       displayed and then saved in the file
%       mixed_posteriors_sp.mat
%
%
...
mode_estimate_data='posterior_mode_sp.mat';
save_final_name='mixed_posteriors_sp';
...
'$\sigma^{eps}_{pc}$'
'$\sigma^{eps}_{mp}$'};
bayesian_plots_and_correlations_model_name='input_example_me_bayesian_correlations_sp';
...
eval(sprintf('eval([''save_name=''''posterior_sp_draw_%s'''';''])', num2str(j)))
[results,draws,results_densities,lagged_expectations]=metropolis_hastings(mode_estimate_data, draw_type, number_of_draws, burn_in, overdispersion,save_name);
