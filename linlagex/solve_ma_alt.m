...
%tri-diagonal system
%
%When the number of entries in the sparse tri-diagonal matrix is large
%"enough", this program uses a different method to build the left-hand side (LHS)
%of the lnear system 
%
...
XXX=zeros(1,(max_it-1)*num_endog*num_eqs*3+4*num_endog*num_eqs);
III=zeros(1,(max_it-1)*num_endog*num_eqs*3+4*num_endog*num_eqs);
JJJ=zeros(1,(max_it-1)*num_endog*num_eqs*3+4*num_endog*num_eqs);
%Initial Entries
%LHS(1:num_eqs,1:2*num_endog)=[B_0,A_0];
XXX(1:2*num_endog*num_eqs)=[B_0(:);A_0(:)];
[II_1,JJ_1]=ind2sub(size([B_0 A_0]), 1:numel(A_0)+numel(B_0));
III(1:2*num_endog*num_eqs)=II_1;
JJJ(1:2*num_endog*num_eqs)=JJ_1;
...
F=F_0;
G=G_0;
[II,JJ]=ind2sub(size([C B A]), 1:numel(A)+numel(B)+numel(C));
...
XXX(2*num_endog*num_eqs+(j-1)*3*num_endog*num_eqs+1:-2*num_endog*num_eqs+2*num_endog*num_eqs+(j)*3*num_endog*num_eqs)=C(:);
XXX(2*num_endog*num_eqs+(j-1)*3*num_endog*num_eqs+1+num_endog*num_eqs:-num_endog*num_eqs+2*num_endog*num_eqs+(j)*3*num_endog*num_eqs)=B(:);
XXX(2*num_endog*num_eqs+(j-1)*3*num_endog*num_eqs+1+2*num_endog*num_eqs:2*num_endog*num_eqs+(j)*3*num_endog*num_eqs)=A(:);
III(2*num_endog*num_eqs+(j-1)*3*num_endog*num_eqs+1:2*num_endog*num_eqs+(j)*3*num_endog*num_eqs)=II+(j)*num_eqs;
JJJ(2*num_endog*num_eqs+(j-1)*3*num_endog*num_eqs+1:2*num_endog*num_eqs+(j)*3*num_endog*num_eqs)=JJ+(j-1)*num_endog;
...
if isnumeric(existence_uniqueness)==1
    ...
    Eyemat=eye(num_endog,num_endog);
    XXX(2*num_endog*num_eqs+(max_it-1)*3*num_endog*num_eqs+1:4*num_endog*num_eqs+(max_it-1)*3*num_endog*num_eqs)=[-ALPHA_ZS(:);Eyemat(:)];
    III(2*num_endog*num_eqs+(max_it-1)*3*num_endog*num_eqs+1:4*num_endog*num_eqs+(max_it-1)*3*num_endog*num_eqs)=II_1+max_it*num_eqs;
    JJJ(2*num_endog*num_eqs+(max_it-1)*3*num_endog*num_eqs+1:4*num_endog*num_eqs+(max_it-1)*3*num_endog*num_eqs)=JJ_1+(max_it-1)*num_endog;
    ...
    LHS=sparse(III,JJJ,XXX,(max_it+1)*num_eqs,(max_it+1)*num_endog);
